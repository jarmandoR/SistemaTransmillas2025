...
<body>
    ...
    <div class="container">
        <div class="uno">
            ...
            ?>
             <button id="toggleSortable">Desactivar Movimiento</button>
 ...
 FROM serviciosdia inner join usuarios on ser_idresponsable=idusuarios inner join cuentaspromotor on cue_idservicio=idservicios where (ser_estado in (3,9) 
 and idservicios not in (SELECT  orden_idservicio FROM ord_recoentregas WHERE orden_iduserencargado='$id_usuario' 
 and orden_fechadiaejecucion='$fechaactual' and ord_tipo in ('Recogida','Entrega'))  $conde3)or (ser_pendientecobrar=1 and ser_estado!=100 and cue_idoperpendiente='$id_usuario')  ORDER BY ser_fechaentrega $asc ";
   ...
   FROM `gastos` inner join usuarios on gas_idusuario=idusuarios inner join sedes on idsedes=gas_idciudaddes
    ...
    <script>
        ...
        console.log($("#drop-items").length); // Debe imprimir 1 si el elemento existe
        $(function() {

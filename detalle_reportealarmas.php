...
if($param35!=''){ $id_sedes=$param35; 
	...
	$conde=" and idsedes=$id_sedes "; 	
}

	
$FB->titulo_azul1("Alarma",1,0,7); 
$FB->titulo_azul1("Fecha Vencimiento",1,0,0); 
$FB->titulo_azul1("Detalles",1,0,0); 

$FB->titulo_azul1("Link de Pago",1,0,0); 
...
if($param36!='0'){ $conde1=" and rep_fechavencimiento<='$fechaactual'";  }
 ...
 $sql="SELECT `idreportealertas`, `rep_alerta`, `rep_fechavencimiento`, `rep_emails`, `rep_useractualiza`, `rep_fechareporte`, `rep_idsede`,sed_nombre,rep_link_pago,rep_banco FROM `reportealertas` inner join sedes on rep_idsede=idsedes WHERE idreportealertas>=0  $conde  $conde1 ORDER BY idreportealertas  asc ";
...
$totalasignadas=0;
	...
	while($rw1=mysqli_fetch_row($DB1->Consulta_ID))
	{
		...
		$id_p=$rw1[0];
			...
			echo "<td>".$rw1[2]."</td>";
			if ($rw1[9]!="") {
				echo "<td colspan='1' width='0' align='center' ><a id='link'  onclick='pop_dis16($id_p,\"Detalles Alarma\",\"$rw1[2]\")';  title='Actualizar Reportes' >ver</td>";
			...
			if ($rw1[8]!='') {
				echo "<td><a href='".$rw1[8]."' target='_blank' >Pagar</a></td>";
			}else {
				echo "<td></td>";
			}
			...
			echo "<td><a href='cambio_admin.php?tabla=Editar Alarma&condecion=$condecion&id=$id_p'>Edit</a></td>";

...
$sql2="SELECT `hoj_fechaingreso`,`hoj_nombre`, `hoj_apellido`, `hoj_fechanacimiento`, `hoj_cedula`,`hoj_celular`, `hoj_sede`,`hoj_estado`,`hoj_cuen`,`hoj_banco`,`hoj_foto`,hoj_cargo,`hoj_tipocontrato`,`hoj_fechacontrato` FROM `hojadevida` WHERE hoj_cedula='$rw[7]' order by idhojadevida LIMIT 1"; 
$DB->Execute($sql2);
...
$FB->llena_texto("Apellido:", 30, 1, $DB, "", "", $rw1[2], 2, 1);
...
$FB->llena_texto("Cargo:",36,2,$DB,"(SELECT `idcargo`, `car_Cargo` FROM `cargo`)", "", $rw1[11], 1, 1);

$FB->llena_texto("Numero de cuenta:", 32, 1, $DB, "", "", $rw1[8], 2, 1);
$FB->llena_texto("Banco:",33, 1, $DB, "", "", $rw1[9], 2, 1);
$FB->llena_texto("Fecha de inicio contrato:", 34, 10, $DB, "", "", "$rw1[0]", 2, 0);
...
echo"<tr><td><label>Crear hoja de vida</label></td><td><input id='param85' name='param85' type='checkbox' ></td></tr>";
...
case "Editar Alarma":
	$id=$_GET['id'];
	$sql = "SELECT `idreportealertas`, `rep_alerta`, `rep_fechavencimiento`, `rep_fechareporte`, 
	`rep_useractualiza`, `rep_emails`, `rep_idsede`, `rep_link_pago`,`rep_banco`,
	`rep_cuenta`,`rep_nombre_consi`,`rep_valor` 
	FROM `reportealertas` 
	WHERE idreportealertas='$id' ";
	$DB->Execute($sql);
	$rw1 = mysqli_fetch_row($DB->Consulta_ID);
		if ($nivel_acceso != '1' and $nivel_acceso != '12') {
			$cond = "and idsedes=$id_sedes";
		}
		$FB->llena_texto("Sede:", 1, 2, $DB, "(SELECT `idsedes`,`sed_nombre` FROM sedes where idsedes>0 $cond  )", "cambio_ajax2(this.value, 15, \"llega_sub1\", \"param2\", 1, 0)", "$rw1[6]", 2, 1);
		$FB->llena_texto("Alarma:", 2, 1, $DB, "", "", "$rw1[1]", 1, 0);
		$FB->llena_texto("Fecha de Vencimiento:", 3, 10, $DB, "", "", "$rw1[2]", 2, 0);
		$FB->llena_texto("E-mail:", 4, 1, $DB, "", "", "$rw1[5]", 1, 0);
		$FB->llena_texto("Documento", 5, 6, $DB, "", "", "", 1, 0);
		$FB->llena_texto("Link de Pago:", 6, 1, $DB, "", "", "$rw1[7]", 1, 0);
		echo "<tr><td><label>Otros</label></td>
      	<td><input id='param85' name='param85' type='checkbox' onchange='toggleOtros()'></td></tr>
		<input name='idalarma' id='idalarma' class='trans'  type='hidden' value='$id'>";
		...
		echo "
			...
			<tr id='otrosInputs7' style='display:$ver' class='text'><td class='text'>Banco:</td><td class='text'><input name='param7' id='param7' class='trans'  type='text' value='$rw1[8]' ></td></tr>
			<tr id='otrosInputs8' style='display:$ver' class='text'><td class='text'>Numero de cuenta:</td><td class='text'><input name='param8' id='param8' class='trans'  type='text' value='$rw1[9]' ></td></tr>
			<tr id='otrosInputs9' style='display:$ver' class='text'><td class='text'>Nombre:</td><td class='text'><input name='param9' id='param9' class='trans'  type='text' value='$rw1[10]' ></td></tr>
			<tr id='otrosInputs10' style='display:$ver' class='text'><td class='text'>Valor:</td><td class='text'><input name='param10' id='param10' class='trans'  type='text' value='$rw1[11]'></td></tr>";
...
function toggleOtros() {
    var checkbox = document.getElementById('param85');
    var otros7 = document.getElementById('otrosInputs7');
	var otros8 = document.getElementById('otrosInputs8');
	var otros9 = document.getElementById('otrosInputs9');
	var otros10 = document.getElementById('otrosInputs10');
    otros7.style.display = checkbox.checked ? 'table-row' : 'none';
	otros8.style.display = checkbox.checked ? 'table-row' : 'none';
	otros9.style.display = checkbox.checked ? 'table-row' : 'none';
	otros10.style.display = checkbox.checked ? 'table-row' : 'none';
}
</script>
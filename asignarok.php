...
// }else{
	...
	$rw2[4]=str_replace("&"," ", $rw2[4]);
	$mensaje3="RECOGIDA: ".$rw2[1]." ".$rw2[4];
	
// }
$fechahoy=date("Y-m-d");
$sql1="SELECT seg_compañero,usu_celular FROM `seguimiento_user` inner join usuarios on idusuarios=seg_compañero WHERE seg_idusuario='$param2' and seg_fechaingreso like '%$fechahoy%'";
...
if ($usu_telefono!='') {
	...
	enviarAlertaWhat("$mensaje3","$usu_telefono",$tipo,"");

	// enviarAlertaWhat("","3125215864",$tipo,"");
}
if ($rw2 === false) {
	echo "No hay resultados (FALSE)"; 
} else {
	$tipo="28";
	enviarAlertaWhat("$mensaje3","$rw2[1]",$tipo,"");
}
